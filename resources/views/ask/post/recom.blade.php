			...
			<div class="aw-content-wrap clearfix">
				<div class="col-sm-12 col-md-9 aw-main-content">
					...
					<ul class="nav nav-tabs aw-nav-tabs active hidden-xs">
						...
						<li class="active"><a href="{{ url('/post/recom') }}">推荐</a></li>
						<li ><a href="{{ url('/post') }}">最新</a></li>
					...
					<div class="aw-mod aw-explore-list">
    						<div class="aw-mod aw-topic-category">
                                ...
                                <ul>
									@if(empty($cid))
                                    <li><a class="active" href="{{ url('/post/recom') }}">全部分类</a></li>
									@else
									<li><a  href="{{ url('/post/recom') }}">全部分类</a></li>
									@endif
                                    @foreach($cates as $cate)
                    					@if($cate->id == $cid)
                    					 <li ><a class="active" style="text-decoration:none;" href="{{ URL::action('Front\PostController@recomCate', ['cid'=>$cate->id]) }}">{{ $cate->name }}</a></li>
                    					@else
                    					 <li><a style="text-decoration:none;" href="{{ URL::action('Front\PostController@recomCate', ['cid'=>$cate->id]) }}">{{ $cate->name }}</a></li>
                    					@endif
                            		@endforeach()
                              	</ul>
                                </div>
                                    ...
                                    @foreach($relateTags as $relateTag)
                                        @if($relateTag->id == $tid)
                                            <span class="topic-tag">
                                                <a style="background-color:red;" class="text" href="{{ URL::action('Front\PostController@recomCate', ['tid'=>$relateTag->id,'cid'=>$cid]) }}" >{{ $relateTag->name }}</a>
                                            </span>
                                        @else
                                            <span class="topic-tag">
                                                <a class="text" href="{{ URL::action('Front\PostController@recomCate', ['tid'=>$relateTag->id,'cid'=>$cid]) }}" >{{ $relateTag->name }}</a>
                                            </span>
                                        @endif
                                    @endforeach()
                                </div>
